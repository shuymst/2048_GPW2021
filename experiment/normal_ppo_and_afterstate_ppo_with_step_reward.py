...
program_dir = "normal_ppo_and_afterstate_ppo_with_step_reward"
figure_dir = f"{program_dir}/figure"
model_dir = f"{program_dir}/model_params"
score_dir = f"{program_dir}/score"
...
for i in range(2):
    ...
    if i == 0:
        torch.save(ppo_agent.model.state_dict(), f"./{program_dir}/model_params/normal_ppo.pth")
    else:
        torch.save(ppo_agent.model.state_dict(), f"./{program_dir}/model_params/afterstate_ppo.pth")
...
np.save(f'{program_dir}/score/score_history_normal', score_history_normal)
np.save(f'{program_dir}/score/score_history_afterstate', score_history_afterstate)
np.save(f'{program_dir}/score/score_history_normal_evaluate', score_history_normal_evaluate)
np.save(f'{program_dir}/score/score_history_afterstate_evaluate', score_history_afterstate_evaluate)
num = 100
b = np.ones(num) / num
...
plt.savefig(os.path.join(figure_dir, 'compare_normal_and_afterstate_step_reward.png'))